...
-- builtins :: [(Ident, Type (Maybe a),[Type b])]
-- builtins = [
...
-- transBuiltin :: (Ident, Type (Maybe a),[Type b]) -> StateT FEnv IO LLVM.TopDef
...
-- transFunDef :: FunDef a-> ReaderT Env (StateT GContext IO) LLVM.TopDef
...
-- storeArg :: (Arg (Maybe a),LLVM.Arg) -> ContT () GenIR ()
...
-- collectFinalBlock :: (Maybe a)-> GenIR a
...
-- transStmt (Incr _ lval) = transStmt $ Ass lval $ EAdd (EVar lval) Plus (ELitInt 1)
-- transStmt (Decr _ lval) = transStmt $ Ass lval $ EAdd (EVar lval) Minus (ELitInt 1)

-- transStmt (Cond _ expr stmt) = transStmt $ CondElse expr stmt Empty    
...
-- transStmt (SExp _ expr) = lift $ transExpr expr >> return ()

-- -- * For type_ ident expr stmt -> failure x

-- transItem :: Item (Maybe a)-> StateT VEnv (ReaderT (Type a) GenIR) ()
-- transItem (NoInit _ ident) = do
--     type_ <- ask
--     transItem $ Init Nothing ident $ defaultValue $ type_ Nothing
...
-- transExpr (Neg _ expr) = transExpr $ EMul Nothing expr Times (ELitInt Nothing $ -1)
